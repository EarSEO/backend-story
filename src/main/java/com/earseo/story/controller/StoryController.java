...
public class StoryController {
    ...
    @Operation(
        summary = "지도 사각형 영역 내 이야기 조회",
        description = """
            지도에서 사각형 영역 내의 이야기 목록을 조회합니다.
            - 좌측 하단(minLongitude, minLatitude)과 우측 상단(maxLongitude, maxLatitude) 좌표를 받습니다.
            - 해당 영역 내에 포함된 모든 이야기 정보를 반환합니다.
    ...
    public ResponseEntity<BaseResponse<SpotStoryPageResponse>> getStoriesRectangleMapInfoList(
        @Parameter(
            description = "사각형 영역의 최소 경도 (좌측 하단)",
            required = true,
            example = "126.9000",
        ...
        return ResponseEntity.ok(BaseResponse.ok(
            storyService.getStoriesRectangleMapInfoList(minLongitude, minLatitude, maxLongitude, maxLatitude, pageRequest.toPageable())
        ));
    }
